...
function searchGoogle(query, cb) {
  google.resultsPerPage = 5;
  google('yelp ' + query.term + ' ' + query.location, function(err, next, links) {
    if (err) console.error(err);

    var patternUrl = new RegExp('http://www.yelp.com/biz/', 'i');
    var patternName = new RegExp('http://www.yelp.com/biz/', 'i');

    var arr = links[0].link.split('/');
    var len = arr.length;

    var id = (patternUrl.test(links[0].link)) ? arr[len - 1] : 'no match';
...
function testAddress(address, input) {
  var re = /^\d+(\s\w+){2,}/;
      ...
      searchYelp(terms, function(biz) {
        ...
        if (typeof biz.businesses !== 'undefined' && biz.businesses.length > 0) {
          ...
          if (request.query.phone) {
            ...
            var businessPhone = _.findWhere(biz.businesses, {
              phone: phone
            });

            if (typeof businessPhone !== 'undefined') {
              match = 'yes';
              businessYelp(businessPhone.id, function(res) {
                reply(res);
              });
            }
          }
          ...
          if (request.query.location) {
            ...
            for (var i = 0, len = biz.businesses.length; i < len; i++) {
              var s = biz.businesses[i].location.address;
              if (testAddress(location, s)) {
                match = 'yes';
                businessYelp(biz.businesses[i].id, function(res) {
                  reply(res);
                });
                break;
              }
